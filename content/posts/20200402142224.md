...
4. 考虑到拖拽时候的拟人轨迹, 使用的是 pyautogui, 所以要前台显示窗口, 网上其他人用的是 selenium 里的 drag offset 方面的东西, 可能支持后台 Headless 拖拽.
   1. 用 chrome cdp 里其实也有, https://chromedevtools.github.io/devtools-protocol/tot/Input#method-dispatchMouseEvent
   2. 不过 ichrome 里还没集成上, 现写原生代码会有点麻烦, 这两天把功能加进去吧
...
def drag(x):
    ...
    pyautogui.moveRel(-10, 1, duration=0.5)
    pyautogui.moveRel(11, -5, duration=0.5)
